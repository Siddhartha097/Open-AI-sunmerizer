    ...
    <section className='mt-8 w-full max-w-xl'>
      <div className='flex flex-col w-full gap-2'>
        <form className='relative flex justify-center items-center' onSubmit={handleSubmit}>
          ...
          />
          <button type='submit' className='submit_btn peer-focus:border-gray-800 peer-focus:text-gray-700'>
          🔎
          </button>
        </form>
          ...
          {allArticles.map((item, index) => (
           <div key={`link-${index}`} onClick={() => setArticle(item)} className='link_card'>
            <div className='copy_btn' onClick={() => handleCopy(item.url)}>
              <img src={copied === item.url ? tick : copy} alt='copy_icon' className='w-[45%] h-[45%] object_contain'/>
            </div>
          ...
          article.summary && (
            <div className='flex flex-col gap-4'>
              <h2 className='font-poppins font-bold text-gray-600 text-xl'>
                Article
                <span className='orange_gradient'> Summary</span>
              </h2>
